...
wash_meta_historic <- read_csv(
  here("analysis", "data", "washington-historic-delays.csv"),
  show_col_types = FALSE
)

wash_meta_historic <- wash_meta_historic |> 
...
wash_time_zero_date <- wash_sample_details$max_date[wash_sample_details$scenario == "full"]
...
This requires you have the metadata for the SARS-CoV-2 sequences used in this analysis.
Visit the data README for the accession ID's to provide for your GISAID query to get this metadata.
...
wash_meta_full <- read_csv(
  here(
    "analysis", 
    "data", 
    "Washignton-SARS-CoV-2-sequence-files",
    "metadata-full.csv"
  ),
  show_col_types = FALSE
)
...
wash_fits$obs_ps_rd_offset <- phylodyn2::BNPR_PS_with_RD(
  wash_trees$obs, 
  lengthout = tree_lengths$grid_lengths[tree_lengths$scenario == "obs"],
  historic_reporting_delays = wash_meta_historic$reporting_delay / 365.25,
  rd_as_offset = TRUE,
  time0_offset_from_sim_rd = wash_sample_details$min_time[wash_sample_details$scenario == "obs"] / 365.25,
  beta1_mean = beta1_mean, 
  beta1_prec = beta1_prec
)

set.seed(1520)
wash_fits$obs_ps_rd_covariate <-  phylodyn2::BNPR_PS_with_RD(
  wash_trees$obs, 
  lengthout = tree_lengths$grid_lengths[tree_lengths$scenario == "obs"],
  historic_reporting_delays = wash_meta_historic$reporting_delay / 365.25,
  rd_as_offset = FALSE,
...
Plot results
```{r produce-real-inference-comparison-plots}
...
```{r save-pref-samp-coefficient-estimates}
write_csv(
  bind_rows(
...
```{r set_up_common_simulation_details_w}
...
```{r simulate-scenario-a-initial-outbreak}
...
```{r produce-plot-scenario-a-comparison}
...
```{r simulate-scenario-b-single-peak}
...
```{r produce-plot-scenario-b-comparison}
# Fix scenario name differences
...
```{r simulate-scenario-c-double-peak}
...
```{r produce-plot-scenario-c-comparison}
# Fix scenario name differences
...
```{r plot-preferential-sampling-coefficient-estimates}
...
```{r set_up_common_simulation_details}
#| label: set_up_common_simulation_details
#| 
time_zeros <- data.frame(
  scenario = c("A", "B", "C"),
  latest_sample_time = c(155, 80, 0)
